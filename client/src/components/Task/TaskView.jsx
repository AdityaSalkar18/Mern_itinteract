...
const TaskView = () => {
  ...
  const [activeModal, setActiveModal] = useState(null); // Track open modal
  ...
  const handleReplyChange = (linkId, linkUId, linkSd, value) => {
    setReplies((prevReplies) => ({
      ...prevReplies,
      [`${linkId}_${linkUId}_${linkSd}`]: value,
    }));
  };
  ...
  const handleReplySubmit = async (e, linkId, linkUId, linkSd) => {
    ...
    try {
      ...
      setReplySuccessMessage("Feedback added successfully");
      setReplies((prevReplies) => ({
        ...prevReplies,
        [`${linkId}_${linkUId}_${linkSd}`]: "",
      }));
      ...
      setReplyError("Error adding feedback");
      console.error("Error adding feedback:", error);
    }
  };
  ...
  useEffect(() => {
    let timer;
    if (replyerror || replysuccessMessage) {
      timer = setTimeout(() => {
        setReplyError("");
        setReplySuccessMessage("");
      }, 2000);
    ...
    return () => clearTimeout(timer);
  }, [replyerror, replysuccessMessage]);
      ...
      <div >
        <div className="container mx-auto px-4 my-8 mt-28 flex justify-center">
          <div className="block w-full max-w-4xl p-6 bg-white border border-gray-200 rounded-lg shadow-sm hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
              ...
              <h6 className='mb-2 text-gray-900 dark:text-white font-medium'>
                {task.tt}
              </h6>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="flex items-center py-2 rounded-lg dark:bg-gray-700">
                ...
                />
                <input
                  className="block p-2 w-full  mb-2 text-sm text-gray-900 bg-white rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-800 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 mx-1 sm:mx-2   sm:mb-0"
                  type="text"
                  name="link"
                  value={formData.link}
        ...
        {/* <div className="container mx-auto px-4 my-8 flex flex-col items-center justify-center">
          {Array.isArray(links) && links.length > 0 ? (
            links.map((link) => (
              <div
                key={link._id}
                className="block w-full max-w-4xl p-4  md:p-6 mb-3 bg-white border border-gray-200 rounded-lg  hover:shadow-xl hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700 transition duration-300 overflow-hidden"
              >
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-400 mb-1">{link.date}</p>
                  </div>
                </div>
                ...
                <p className=" mb-2 break-words dark:text-white">
                  {link.desc}
                </p>
                <div className="flex justify-end">
                  <Link
                    onClick={openModal}
                    data-bs-target={`crud-modal-${link._id}`}
                    className="px-4 py-2 text-sm font-medium text-white bg-[#005A9C] rounded-lg hover:bg-[#004080] focus:outline-none focus:ring-2 focus:ring-[#66A3D2] dark:bg-[#005A9C] dark:hover:bg-[#003366] dark:focus:ring-[#003366] transition duration-300"
                  >
                    Feedback
                  </Link>
                </div>
      ...
      {Array.isArray(links) && links.length > 0 ? (
        links.map((link) => (
          ...
          >
            ...
            <p className="mb-2 break-words dark:text-white">{link.desc}</p>
            <div className="flex justify-end">
              <button
                  ...
                  <div className="relative bg-white rounded-lg shadow-lg dark:bg-gray-700">
                    {replyerror && (
                      <div
                        ...
                        role="alert"
                      >
                        <span className="font-medium">Error: </span> {replyerror}
                      </div>
                    )}
                    {replysuccessMessage && (
                      <div
                    ...
                    <form
                      onSubmit={(e) =>
                        handleReplySubmit(e, link._id, link.upid, link.tasksd)
                      }
                    >
                      <div className="mb-6 p-4">
                        <label
                        ...
                        <select
                          ...
                          className="w-full px-3 py-2 text-sm border rounded-lg bg-gray-50 dark:bg-gray-700 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                          value={
                            replies[`${link._id}_${link.upid}_${link.tasksd}`] ||
                            ""
                          }
                          onChange={(e) =>
                            handleReplyChange(
                      ...
                      <div className="flex justify-end">
                        <button
                          type="submit"
                          className="px-4 py-2 m-2 text-sm font-medium text-white bg-[#005A9C] rounded-lg hover:bg-[#004A82]"
                        >
                          Submit
                        </button>
                      </div>
