...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = "http://localhost:8080/api/api/users/student";
      const { data: res } = await axios.post(url, data);
      navigate("/");
      console.log(res.message);
    } catch (error) {
      ...
      <div className="container py-2">
        ...
        <div class="col-md-8 mx-auto col-lg-4">
          <form class="p-4 p-md-5 border rounded-3 bg-body-tertiary" onSubmit={handleSubmit}>
            <h3 className='mb-3'>Signup With Student Account</h3>
            ...
            <div class="form-floating mb-3">
              ...
              <label for="floatingInput">Vcet Email address</label>
            </div>
            <div class="form-floating mb-3">
            ...
            <hr class="my-4" />
             ...
             <Link to="/userVerification" className='mb-2'>
            <button
              className="w-80 mb-2 btn btn-outline-secondary"
            >
              Signup With User Account
            </button>
            </Link>
